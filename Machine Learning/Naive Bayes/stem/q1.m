tic
ftrain=importdata('nb_data\r8-train-all-terms-new.txt');     %read text file of train data
...
%Test data
ftest=importdata('nb_data\r8-test-all-terms-new.txt');     %read text file of test data
...
dict=union(dict,dict2);                                 %dictionary of words in test data
...
accuracyTrain=((sum(Htrain==YTrain))/m1)*100
%pause
...
accuracyTest=((sum(Htest==YTest))/m2)*100
%pause
%random prediction of test set
Hrand=randi(8,m2,1);
Hrand=uniqY(Hrand);
random_accuracy=((sum(Hrand==YTest))/m2)*100

%majority prediction of test set
majo=mode(YTrain);
Hmajo(1:m2,1)=majo;
majority_accurcy=((sum(Hmajo==YTest))/m2)*100
improvementOverRand=accuracyTest-random_accuracy
improvementOverMajo=accuracyTest-majority_accurcy
%pause
%confusion Matrix Formation
ConfusionMat=confusionMatrix(YTest,Htest)
